    ...
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
      <div className="relative w-[400px] h-[400px]">
        ...
        >
          ...
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-white/20" />
          <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white/35" />
        </motion.div>

        <motion.div
          animate={{ y: [0, -20, 0], rotate: [0, 10, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-12 right-12 w-20 h-20 rounded-2xl bg-gradient-to-br from-white/30 to-white/10 backdrop-blur-sm border border-white/20"
        />

        <motion.div
          animate={{ y: [0, 15, 0], rotate: [0, -15, 0] }}
          transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
          className="absolute bottom-20 left-8 w-16 h-16 rounded-xl bg-gradient-to-br from-white/25 to-white/5 backdrop-blur-sm border border-white/15"
        />

        <motion.div
          animate={{ y: [0, -10, 0], x: [0, 10, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }}
          className="absolute top-1/3 left-16 w-12 h-12 rounded-lg bg-white/20 backdrop-blur-sm"
        />
        ...
        >
          <motion.div
            ...
            className="w-48 h-64 bg-white/15 backdrop-blur-xl rounded-3xl border border-white/30 p-5 shadow-2xl"
          >
            <div className="w-12 h-12 rounded-xl bg-white/30 mb-4" />
            ...
            <motion.div
              ...
              className="absolute -top-3 -right-3 w-14 h-14 bg-white rounded-2xl flex flex-col items-center justify-center shadow-xl"
            >
              <span className="text-[#2F6BFF] text-lg font-bold">92</span>
              <span className="text-slate-400 text-[8px]">SCORE</span>
            </motion.div>
          </motion.div>
        </motion.div>
        ...
        >
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-white/60" />
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-white/40" />
        </motion.div>

        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
          transition={{ duration: 3, repeat: Infinity }}
          className="absolute bottom-8 right-20 w-24 h-24 rounded-full border-2 border-white/20"
        />

        <motion.div
          animate={{ rotate: [0, 180, 360] }}
          transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
          className="absolute top-20 left-1/3 w-8 h-8"
        >
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#2F6BFF] via-[#4B7FFF] to-[#3CE0B1] relative overflow-hidden">
          ...
          <div className="absolute bottom-12 left-12 text-white">
            ...
            <p className="text-white/70 text-sm max-w-[240px]">
              AI-powered CV optimization for modern professionals.
            </p>
          </div>
        </div>
          ...
          >
            <motion.div variants={itemVariants} className="lg:hidden flex justify-center mb-8">
              <Link href="/" className="flex items-center gap-2">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#2F6BFF] to-[#3CE0B1] flex items-center justify-center">
                  <Sparkles size={22} className="text-white" />
                </div>
            ...
            <motion.div variants={itemVariants}>
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full h-14 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 hover:border-[#2F6BFF] font-medium rounded-xl flex items-center justify-center gap-3 transition-colors mb-6 disabled:opacity-50"
              >
            ...
            <motion.p variants={itemVariants} className="mt-8 text-center text-sm text-slate-500">
              ...
              <Link href="/register" className="text-[#2F6BFF] hover:text-[#3CE0B1] font-semibold transition-colors">
                Create one
              </Link>
            </motion.p>
          </motion.div>
