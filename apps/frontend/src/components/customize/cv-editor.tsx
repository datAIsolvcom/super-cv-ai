...
export function CvEditor() {
  ...
  return (
    <div 
        ...
        className={cn(
            "bg-white text-slate-900 w-full h-full",
            design.fontFamily, 
            design.pageMargin,
            design.fontSize,
            design.lineHeight
...
// --- HELPERS (SAMA) ---
function SectionHeader({ title, design, className }: any) { return <div className={cn("flex justify-between items-center mb-2 border-b pb-1", className)} style={{ borderColor: design.accentColor }}><h2 className="text-[0.9em] font-bold uppercase tracking-widest" style={{ color: design.accentColor }}>{title}</h2></div> }
function renderProjects(cvData: any, design: any, update: any, add: any, remove: any, uiOnly: string, preventBreak: string, keepWithNext: string) { return (cvData.projects || []).length > 0 ? ( <div className="group relative pl-4 hover:border-l-4 hover:border-blue-200 transition-all rounded p-2 hover:bg-slate-50 border-l-4 border-transparent"> <div className={`flex justify-between items-center mb-2 border-b pb-1 ${keepWithNext}`} style={{ borderColor: design.accentColor }}> <h2 className="text-[0.9em] font-bold uppercase tracking-widest" style={{ color: design.accentColor }}>Projects</h2> <button onClick={() => add('projects')} className={`opacity-0 group-hover:opacity-100 text-blue-500 ${uiOnly}`}><Plus size={16}/></button> </div> <div className={cn("flex flex-col", design.sectionSpacing)}> {(cvData.projects || []).map((proj: any, i: number) => ( <div key={i} className={`group/item relative ${preventBreak}`}> <button onClick={() => remove('projects', i)} className={`absolute -right-6 top-0 opacity-0 group-hover/item:opacity-100 text-red-400 p-1 ${uiOnly}`}><Trash2 size={14}/></button> <h3 className="font-bold text-slate-900 text-[1.15em] leading-tight"><EditableField value={proj.name} onSave={(v) => update(`projects[${i}].name`, v)}/></h3> <div className="text-slate-600 mb-1"><EditableField tagName="p" value={proj.description} onSave={(v) => update(`projects[${i}].description`, v)}/></div> <div className="flex flex-col gap-0.5"> {(proj.highlights || []).map((h: string, idx: number) => ( <div key={idx} className="flex items-start group/bullet relative"> <span className="mt-[0.6em] mr-2 h-1.5 w-1.5 shrink-0 rounded-full bg-slate-800 print:bg-black" /> <div className="flex-1 text-slate-700 text-justify"> <EditableField tagName="span" value={h} className="inline" onSave={(v) => { const nh = [...proj.highlights]; nh[idx] = v; update(`projects[${i}].highlights`, nh); }}/> </div> <button onClick={() => { const nh = proj.highlights.filter((_: any, x: number) => x !== idx); update(`projects[${i}].highlights`, nh); }} className={`opacity-0 group-hover/bullet:opacity-100 text-red-300 ml-2 ${uiOnly}`}><Trash2 size={12}/></button> </div> ))} <button onClick={() => update(`projects[${i}].highlights`, [...proj.highlights, "New"])} className={`text-blue-400 hover:underline opacity-0 group-hover/item:opacity-100 text-left ml-4 mt-1 ${uiOnly}`}>+ Add highlight</button> </div> </div> ))} </div> </div> ) : null; }
function renderEducation(cvData: any, design: any, update: any, add: any, remove: any, uiOnly: string, preventBreak: string, keepWithNext: string) { return (cvData.education || []).length > 0 ? ( <div className="group relative pl-4 hover:border-l-4 hover:border-blue-200 transition-all rounded p-2 hover:bg-slate-50 border-l-4 border-transparent"> <div className={`flex justify-between items-center mb-2 border-b pb-1 ${keepWithNext}`} style={{ borderColor: design.accentColor }}> <h2 className="text-[0.9em] font-bold uppercase tracking-widest" style={{ color: design.accentColor }}>Education</h2> <button onClick={() => add('education')} className={`opacity-0 group-hover:opacity-100 text-blue-500 ${uiOnly}`}><Plus size={16}/></button> </div> <div className={cn("flex flex-col", design.sectionSpacing)}> {(cvData.education || []).map((edu: any, i: number) => ( <div key={i} className={`group/item relative ${preventBreak}`}> <button onClick={() => remove('education', i)} className={`absolute -right-6 top-0 opacity-0 group-hover/item:opacity-100 text-red-400 p-1 ${uiOnly}`}><Trash2 size={14}/></button> <div className="flex justify-between items-baseline"> <EditableField tagName="h3" className="font-bold text-slate-900 text-[1.15em] leading-tight" value={edu.institution} onSave={(v) => update(`education[${i}].institution`, v)}/> <EditableField tagName="span" className="text-[0.9em] text-slate-500" value={edu.year} onSave={(v) => update(`education[${i}].year`, v)}/> </div> <EditableField tagName="div" className="text-slate-700" value={edu.degree} onSave={(v) => update(`education[${i}].degree`, v)}/> </div> ))} </div> </div> ) : null; }