...
export function RibbonBar() {
  ...
  return (
    <>
      ...
      {activeDropdown && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 md:hidden" onClick={() => setActiveDropdown(null)} />
      )}
            ...
            {activeDropdown === 'font' && (
                <div className={cn(dropdownBase, "left-0")}>
                    <div>
                        ...
                        {fonts.map(f => (
                            <button key={f.key} onClick={() => { setDesign({...design, fontFamily: f.key as any}); }} className={`w-full text-left px-2 py-1.5 rounded-lg text-sm flex justify-between items-center text-slate-900 ${design.fontFamily === f.key ? 'bg-slate-100 font-bold' : 'hover:bg-slate-50'}`}>
                    ...
                    <div>
                        ...
                        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                             {fontSizes.map(s => ( <button key={s.key} onClick={() => setDesign({...design, fontSize: s.key})} className={`flex-1 py-1 text-xs rounded-md ${design.fontSize === s.key ? 'bg-white shadow text-black font-bold' : 'text-slate-500 hover:text-black'}`}>{s.label}</button> ))}
                        </div>
                    </div>
                </div>
            )}
           ...
           <div className="relative">
               ...
               {activeDropdown === 'color' && (
                   <div className={cn(dropdownBase, "left-1/2 -translate-x-1/2 w-48 grid-cols-4")}>
                       {colors.map(c => ( <button key={c} onClick={() => { setDesign({...design, accentColor: c}); setActiveDropdown(null); }} className="w-8 h-8 rounded-full border border-slate-200 hover:scale-110 transition-transform shadow-sm mx-auto" style={{ backgroundColor: c }} /> ))}
                   </div>
               )}
           </div>
               ...
               {activeDropdown === 'layout' && (
                   <div className={cn(dropdownBase, "left-1/2 -translate-x-1/2 w-64")}>
                       <div>
                           ...
                           <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                               {margins.map(m => ( <button key={m.key} onClick={() => setDesign({...design, pageMargin: m.key})} className={`flex-1 py-1.5 text-xs rounded-md transition-all ${design.pageMargin === m.key ? 'bg-white shadow text-black font-bold' : 'text-slate-500 hover:text-black'}`}>{m.label}</button> ))}
                           </div>
                       </div>
                       <div>
                           <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Spacing</label>
                           <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                               {spacings.map(s => ( <button key={s.key} onClick={() => setDesign({...design, sectionSpacing: s.key})} className={`flex-1 py-1.5 text-xs rounded-md transition-all ${design.sectionSpacing === s.key ? 'bg-white shadow text-black font-bold' : 'text-slate-500 hover:text-black'}`}>{s.label}</button> ))}
                           </div>
                       </div>
                       <div>
                           <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block flex items-center gap-2"><AlignJustify size={10}/> Line Height</label>
                           <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                               {lineHeights.map(l => ( <button key={l.key} onClick={() => setDesign({...design, lineHeight: l.key})} className={`flex-1 py-1.5 text-xs rounded-md transition-all ${design.lineHeight === l.key ? 'bg-white shadow text-black font-bold' : 'text-slate-500 hover:text-black'}`}>{l.label}</button> ))}
                           </div>
                       </div>
                   </div>
               )}
