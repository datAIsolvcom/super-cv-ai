...
export function CvEditor({ printRef, highlightSection, isPreviewMode = false }: CvEditorProps) {
  ...
  return (
    ...
    >
      <header className={`border-b-2 pb-6 mb-4 flex flex-col items-center text-center ${template === "minimal" ? "items-start text-left border-none" : ""}`} style={{ borderColor: design.accentColor }}>
        ...
        <div className={`flex flex-wrap gap-x-4 gap-y-2 text-[0.9em] text-slate-600 ${template === "minimal" ? "justify-start" : "justify-center"}`}>
          {contact.email && <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Mail size={14} style={{ flexShrink: 0 }} /><Field tagName="span" value={contact.email} onSave={(v) => updateCvField("contact_info.email", v)} /></div>}
          {contact.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Phone size={14} style={{ flexShrink: 0 }} /><Field tagName="span" value={contact.phone} onSave={(v) => updateCvField("contact_info.phone", v)} /></div>}
          {contact.location && <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><MapPin size={14} style={{ flexShrink: 0 }} /><Field tagName="span" value={contact.location} onSave={(v) => updateCvField("contact_info.location", v)} /></div>}
          {contact.linkedin && <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Linkedin size={14} style={{ flexShrink: 0 }} /><Field tagName="span" value={contact.linkedin} onSave={(v) => updateCvField("contact_info.linkedin", v)} /></div>}
          {contact.portfolio && <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Globe size={14} style={{ flexShrink: 0 }} /><Field tagName="span" value={contact.portfolio} onSave={(v) => updateCvField("contact_info.portfolio", v)} /></div>}
        </div>
      </header>
