...
export function AnalysisView({ analysisResult }: AnalysisViewProps) {
    ...
    // Sticky loading: Keeps loader visible across mutation -> navigation gap
    ...
    const handleUnlock = async () => {
        if (!session?.user?.id) {
            // Smart redirect: preserve the current analysis page path
    ...
    const handleCustomize = async (mode: "analysis" | "job_desc") => {
        // Set sticky loading message - this stays until we navigate away
        ...
        customizeMutation.mutate(
            ...
            {
                onSuccess: () => {
                    // Keep stickyMessage ON - it will unmount when navigation happens
                    router.push(`/cv/${cvId}/editor`);
                },
                onError: (error) => {
                    console.error("AI Customization error:", error);
                    setStickyMessage(null); // Clear only on error
                },
            }
        );
    };
    ...
    // ============================================
    // PRIORITY 1: Sticky loading (user action) - takes precedence
    // ============================================
            ...
            <div className="min-h-screen flex items-center justify-center bg-slate-950">
                <div className="flex flex-col items-center justify-center">
                    <motion.div
                        ...
                        className="w-20 h-20 bg-gradient-to-br from-champagne-400 to-orange-600 rounded-full flex items-center justify-center mb-8 shadow-[0_0_60px_rgba(245,158,11,0.4)]"
                    >
                    ...
                    <motion.p
                        ...
                        className="text-white text-xl font-serif font-bold tracking-wide"
                    >
    ...
    return (
        <div className="max-w-7xl mx-auto w-full relative pb-20">
            ...
            {/* Full-screen loading overlay for AI customization */}
            <AnimatePresence>
                {stickyMessage && (
                    <motion.div
                        initial={{ opacity: 0 }}
            ...
            {!isUnlocked ? (
                <div className="max-w-4xl mx-auto py-20 px-4 text-center relative min-h-[600px] flex flex-col items-center justify-center">
                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900/80 backdrop-blur-2xl p-10 md:p-14 rounded-[40px] shadow-2xl border border-champagne-500/20 max-w-lg w-full relative z-10">
                        ...
                        <button onClick={handleUnlock} disabled={claimMutation.isPending} className="w-full py-5 bg-white text-slate-900 font-bold text-lg rounded-2xl hover:bg-champagne-100 transition-all transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 shadow-xl">
                            {claimMutation.isPending ? <Loader2 className="animate-spin text-orange-600" /> : <Unlock size={24} className="text-orange-600" />}
                            ...
                            <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
                                <div className="flex-1 space-y-6 text-center md:text-left">
                                    ...
                                    <div className="flex flex-wrap gap-4 justify-center md:justify-start pt-2">
                                        <button onClick={() => handleCustomize("analysis")} disabled={customizeMutation.isPending} className="px-8 py-3 bg-white text-slate-950 font-bold rounded-xl hover:bg-champagne-100 transition-colors shadow-lg shadow-white/10 flex items-center gap-2">
                                            {customizeMutation.isPending ? <Loader2 size={18} className="animate-spin" /> : <Zap size={18} className="text-orange-600" />}
                                            Fix Issues with AI
                                        </button>
